...
centre_cue1 = visual.ShapeStim(
    ...
    size=(0.5, 0.5), vertices='triangle',
    ori=90.0, pos=(0.25, 0), anchor='center',
    lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
...
probe_r = visual.ShapeStim(
    ...
    size=(2, 2), vertices='circle',
    ori=0.0, pos=[0,0], anchor='center',
    lineWidth=8.0,     colorSpace='rgb',  lineColor=[-1.0000, -1.0000, 1.0000], fillColor=[0.0000, 0.0000, 0.0000],
    opacity=None, depth=-3.0, interpolate=True)
...
for thisTrial in trials:
    ...
    if stim_side == 10:
        ...
        if side == 1:
            ...
            if neutral_side == 1:
                ...
                l_fill_color = (0,0,0,0)    
    # keep track of which components have finished
    cueComponents = [left_cue, right_cue, fc, p_port, centre_cue1, centre_cue2]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if left_cue.status == STARTED:
            ...
            if tThisFlipGlobal > left_cue.tStartRefresh + 2-frameTolerance:
        ...
        if right_cue.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > right_cue.tStartRefresh + 2-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if centre_cue1.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > centre_cue1.tStartRefresh + 2-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if centre_cue2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > centre_cue2.tStartRefresh + 2-frameTolerance:
                # keep track of stop time/frame for later
    ...
    continueRoutine = True
    # update component parameters for each repeat
    #outlet.push_sample([f'trial_{side}'])
    ...
    while continueRoutine:
        ...
        if probe_l.status == STARTED:  # only update if drawing
            probe_l.setLineColor('blue', log=False)
        ...
        if p_port_2.status == NOT_STARTED and t >= 0.0-frameTolerance:
            ...
            win.callOnFlip(p_port_2.setData, int(55))
