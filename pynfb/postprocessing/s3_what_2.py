...
for j, ch in enumerate(ch_plot):
    leg = []
    for jj, key in enumerate(['1. Opened', '3. Baseline', '15. Baseline', '4. FB', '6. FB', '8. FB','10. FB','12. FB',
                              '2. Left', '14. Left']):
        x = raw_before[key]
        style = '' if key[0]!='1' else '--'
        w = 2
        if 'FB' in key:
            style = ''
            w = jj-2
        f, Pxx = welch(x[:, channels.index(ch)], fs, nperseg=2048,)
        axes[j].plot(f, Pxx, style,  c=cm(key), linewidth=w)
        x_plot = np.abs(hilbert(fft_filter(x[:, channels.index(ch)], fs)))
        leg.append('{2}: P={0:.3f}, D={1:.2f}s'.format(Pxx[(f>9) & (f<14)].mean(), sum((x_plot > 5))/fs, key))
