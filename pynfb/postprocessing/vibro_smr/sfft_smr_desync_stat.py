...
band_name = 'high-beta'
block2_name = 'rest-after'
block1_name = 'rest-before'
...
des = pd.DataFrame(columns=['D', 'condition', 'subj', 'band_name'])
specs = pd.DataFrame(columns=['D', 'condition', 'subj', 'freq.'])
...
for n_exp in list(range(0 , len(experiments)))[::-1]:
    ...
    f, t, Sxx = signal.spectrogram(x, fs, scaling='spectrum', nperseg=fs, nfft=fs, noverlap=int(fs*0.75))
    #print(Sxx)
    Sxx = Sxx**0.5
    ...
    specs = pd.concat([specs, pd.DataFrame({'D': desyn, 'condition': 'control' if exp['control'] else 'exp', 'subj': exp['name'],
                         'freq.': f})])
    ...
    for band_name, band in bands:
        ...
        desyn = (rest - motor)/rest
        des.loc[len(des)] = {'D': desyn, 'condition': 'control' if exp['control'] else 'exp', 'subj': exp['name'], 'band_name':band_name}
...
sns.tsplot(specs, 'freq.', 'subj', 'condition', 'D')

#sns.tsplot(specs, 'freq.', 'subj', 'condition', 'D', err_style='unit_traces')
plt.show()

sns.boxplot(x='band_name', y='D', hue='condition',  data=des)
plt.ylim(-0.5, 0.5)


for j, (band_name, band) in enumerate(bands):
    des_band = des.loc[des['band_name'] == band_name]
    p_v = ranksums(des_band.loc[des['condition']=='exp', 'D'], des_band.loc[des['condition']=='control', 'D']).pvalue
    plt.text(j, 0.5, "{} {:.3f}".format('ns' if p_v > 0.05 else ('**' if p_v<0.01 else '*'), p_v), ha='center', va='bottom')
#sns.swarmplot(x='band_name', y='D', hue='condition', data=des, color='k', alpha=0.5)
from scipy.stats import ttest_ind
#print(ttest_ind(des.loc[des['condition']=='exp', 'D'], des.loc[des['condition']=='control', 'D']))
#print(ttest_rel(des.loc[des['condition']=='exp', 'D'], des.loc[des['condition']=='control', 'D']))
#print(wilcoxon(des.loc[des['condition']=='exp', 'D'], des.loc[des['condition']=='control', 'D']))
#plt.title('Ranksums p-value:')
print(ranksums(des.loc[des['condition']=='exp', 'D'], des.loc[des['condition']=='control', 'D']))
plt.show()