...
xx = np.concatenate([raw_before['Baseline'], raw_after['Baseline'], raw_before['Left'], raw_after['Left'], ])
...
print(spatial)
plot_topomap(unmixing_matrix[:, 0], ch_names_to_2d_pos(channels))
...
cm = get_colors()
for name, x in list(raw_before.items()) + list(raw_after.items()):
    if name in ['Closed','Opened', 'Right', 'Left', 'Baseline']:
        for j, ch in enumerate(ch_plot):
            ...
            axes[j].plot(time, fft_filter(np.dot(x, spatial), fs, band=(3, 45)), c=cm[name], alpha=1, linewidth=1)
