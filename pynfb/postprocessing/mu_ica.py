...
# plot raw data
ch_plot = ['C4', 'P4', 'ICA', 'CSP']#, 'Pz', 'Fp1']
fig1, axes = plt.subplots(len(ch_plot), ncols=1, sharex=True, sharey=Fals)
print(axes)
...
for name, x in list(raw_before.items()):
    for j, ch in enumerate(ch_plot):
        ...
        x_plot = fft_filter(y, fs, band=(9, 14)) if ch != 'Fp1' else x[:, channels.index(ch)]
...
for j, ch in enumerate(ch_plot):
    leg = []
    for jj, key in enumerate(['14. Baseline', '10. Open', '11. Left', '15. FB', '17. FB', '19. FB','21. FB','23. FB',
                              '26. Open', '27. Left']):
                              #'2. Left', '14. Left']):
        x = raw_before[key]
        style = '--' if key in ['26. Open', '27. Left'] else ''
        w = 2
        if 'FB' in key:
            style = ''
            w = jj-1
        ...
        x_plot = np.abs(hilbert(fft_filter(y, fs)))
        threshold = 1.5 * x_median[channels.index(ch)] if ch not in ['ICA', 'CSP'] else 1.5 * x_f_median if ch == 'ICA' else 1.5 * x_csp_median
