...
from ..protocols.widgets import (CircleFeedbackProtocolWidgetPainter, BarFeedbackProtocolWidgetPainter,
                                 ...
                                 PosnerFeedbackProtocolWidgetPainter, EyeTrackFeedbackProtocolWidgetPainter)
...
class Protocol:
    def __init__(self, signals, source_signal_id=None, name='', duration=30, update_statistics_in_the_end=False,
                 ...
                 random_over_time=0, show_probe=False, probe_duration=0, probe_loc='RAND', show_pc_score_after=0, posner_test=False, eye_range=100, enable_posner=False):
        """ Constructor
...
class FeedbackProtocol(Protocol):
    def __init__(self, signals, name='Feedback', circle_border=0, m_threshold=1, gabor_theta=45, **kwargs):
        ...
        elif circle_border == 6:
            self.widget_painter = EyeTrackFeedbackProtocolWidgetPainter(show_reward=self.show_reward,
                                                                        m_threshold=m_threshold,
                                                                        r_threshold=self.reward_threshold,
                                                                        center_fixation=0)
        else:
...
class EyeCalibrationProtocol(Protocol):
    def __init__(self, signals, name='EyeCalibration', **kwargs):
        kwargs['name'] = name
        super().__init__(signals, **kwargs)
        self.widget_painter = EyeCalibrationProtocolWidgetPainter(protocol_duration=self.freq*self.duration)
